...
pokeprofile* gotonum(int num);
pokemove* gotomove(std::string name);
pokemove* gotoend(void);
...
void moveinfo(pokemove *p);
void pokelist(void);
void cry(pokeprofile *p);

bool raised(pokeprofile *p);
bool hit(pokemove *m);
bool stab(pokeprofile *att, pokemove *m);
int typefx(pokeprofile *def, pokemove *mv);
bool criticalhit(pokeprofile *att, pokemove *m);
...
pokemove *moveslist;
player trainer[2];
int bg=0, active[2], sound_volume=50;
std::string msg="";
...
int main()
{
    ...
    moveslist=(pokemove*)malloc(sizeof(pokemove));
    moveslist->nxtmove=0;

    srand(time(0));
    window.create(sf::VideoMode(1280, 720), "Pokémon Mini BattleGround");
    icon();
    ...
    while(window.isOpen())
    {
        ...
        if(choice==1)
        {
            ...
            while(!lose(&trainer[1]) && !lose(&trainer[0]) && window.isOpen()) //While the entire match is not over
            {
                ...
                while(trainer[1].team[active[1]].hp && trainer[0].team[active[0]].hp && window.isOpen()) //While the Pokémon are not knocked out
                {
                    turn++;
                    activity=5;
                    ...
                    while(activity>4 && window.isOpen())
                    {
                        while(window.pollEvent(event))
                        {
                            ...
                            if(event.type==sf::Event::MouseMoved)
                            {
                                ...
                                if(YCursor>=600 && YCursor<=700)
                                {
                                    high=5;
                                    if(XCursor>=120 && XCursor<=600) high=1;
                                    else if(XCursor>=760 && XCursor<=1240) high=2;
                                    high+=2*(YCursor>=650);
                            ...
                            if(event.type==sf::Event::MouseButtonPressed && event.mouseButton.button==sf::Mouse::Left && YButton>=600 && YButton<=700)
                            {
                                if(XButton>=120 && XButton<=600) activity=1;
                                else if(XButton>=760 && XButton<=1240) activity=2;
                                activity+=2*(YButton>=650);
                            }
                        }
                    }
                    if(!window.isOpen()) break;
                ...
                if(!quit) //If Pokémon is knocked out
                {
                    std::string str="";
            ...
            clk.restart();
            while(clk.getElapsedTime().asSeconds()<30 && window.isOpen())
            {
                bool flag=0;
    ...
    while(pokedex->nxt)
    {
        pokeprofile* del=pokedex->nxt;
        pokedex->nxt=del->nxt;
        free(del);
    }
    free(pokedex);

    while(moveslist->nxtmove)
    {
        pokemove* del=moveslist->nxtmove;
        moveslist->nxtmove=del->nxtmove;
        free(del);
    }
    free(moveslist);
...
{
    pokeprofile *p;
    for(p=pokedex->nxt; p && num--; p=p->nxt);
    return p;
}
...
void addDexentry(int id)
{
    pokeprofile *p=gotoid(id);
    if(p)
    {
        std::string txt="dexentry/"+std::to_string(id)+".txt";
        ...
        for(int i=3; entry[i]; i++) if(entry[i-3]=='P') if(entry[i-2]=='o') if(entry[i-1]=='k') if(entry[i]=='e') entry[i]='é';
        strcpy(p->dexentry, entry);
    }
}

pokemove* gotoend(void)
{
    pokemove *p=moveslist;
...
{
    pokemove *p;
    for(p=moveslist->nxtmove; p && strcmp(p->name, name.c_str()); p=p->nxtmove);
    return p;
}
...
{
    pokemove *newmove;
    newmove=(pokemove*)malloc(sizeof(pokemove));
    ...
    while(window.isOpen())
    {
        while(window.pollEvent(event))
        {
            ...
            if(event.type==sf::Event::KeyPressed)
            {
                ...
                if(event.key.code==sf::Keyboard::Right && sound_volume<100) sound_volume+=2, setv=1;
                if(event.key.code==sf::Keyboard::Left && sound_volume) sound_volume-=2, setv=1;

                if(setv) music[1].setVolume(sound_volume/2);
            }

            if(event.type==sf::Event::MouseMoved)
            {
                if(XCursor>=100 && XCursor<=750)
                {
                    if(YCursor>=60 && YCursor<=240)
                    {
                    ...
                    else if(YCursor>=270 && YCursor<=450)
                    {
                        s1.setPosition(100, 60);
                    ...
                    else if(YCursor>=480 && YCursor<=660)
                    {
                        s1.setPosition(100, 60);
                        s2.setPosition(100, 270);
            ...
            if(event.type==sf::Event::MouseButtonPressed && event.mouseButton.button==sf::Mouse::Left && XButton>=100 && XButton<=750)
            {
                if(YButton>=60 && YButton<=240) return 1;
                if(YButton>=270 && YButton<=450) return 2;
                if(YButton>=480 && YButton<=660) return 0;
            }
        }
...
void rotomdex(void)
{
    ...
    sf::Texture texture, t1, t2, t3, t4, t5;
    sf::Sprite sprite, s1, s2, s3, s4, s5;

    texture.loadFromFile("img/menu/bg.jpg");
    sprite.setTexture(texture);
    t1.loadFromFile("img/menu/dex1.jpg");
    s1.setTexture(t1);
    t2.loadFromFile("img/menu/dex2.jpg");
    s2.setTexture(t2);
    t3.loadFromFile("img/menu/dex3.jpg");
    s3.setTexture(t3);
    t4.loadFromFile("img/menu/dex4.jpg");
    s4.setTexture(t4);
    t5.loadFromFile("img/menu/dex5.jpg");
    s5.setTexture(t5);

    s1.setPosition(30, 45);
    s2.setPosition(30, 240);
    s3.setPosition(420, 240);
    s4.setPosition(30, 495);
    s5.setPosition(420, 495);
    ...
    while(choice && window.isOpen())
    {
        ...
        while(window.pollEvent(event))
        {
            ...
            if(event.type==sf::Event::MouseMoved)
            {
                if(XCursor>=30 && XCursor<=780 && YCursor>=45 && YCursor<=165)
                {
                    s1.setPosition(60, 45);
                    s2.setPosition(30, 240);
                    s3.setPosition(420, 240);
                    s4.setPosition(30, 495);
                    s5.setPosition(420, 495);
                ...
                else if(XCursor>=30 && XCursor<=360 && YCursor>=225 && YCursor<=405)
                {
                    s1.setPosition(30, 45);
                    s2.setPosition(30, 225);
                    s3.setPosition(420, 240);
                    s4.setPosition(30, 495);
                    s5.setPosition(420, 495);
                }

                else if(XCursor>=420 && XCursor<=750 && YCursor>=225 && YCursor<=405)
                {
                    s1.setPosition(30, 45);
                    s2.setPosition(30, 240);
                    s3.setPosition(420, 225);
                    s4.setPosition(30, 495);
                    s5.setPosition(420, 495);
                }

                else if(XCursor>=30 && XCursor<=360 && YCursor>=480 && YCursor<=675)
                {
                    s1.setPosition(30, 45);
                    s2.setPosition(30, 240);
                    s3.setPosition(420, 240);
                    s4.setPosition(30, 480);
                    s5.setPosition(420, 495);
                }

                else if(XCursor>=420 && XCursor<=750 && YCursor>=480 && YCursor<=675)
                {
                    s1.setPosition(30, 45);
                    s2.setPosition(30, 240);
                    s3.setPosition(420, 240);
                    s4.setPosition(30, 495);
                    s5.setPosition(420, 480);
                }

                else if(s1.getPosition().x>30 || s2.getPosition().y<240 || s3.getPosition().y<240 || s4.getPosition().y<495 || s5.getPosition().y<495)
                {
                    s1.setPosition(30, 45);
                    s2.setPosition(30, 240);
                    s3.setPosition(420, 240);
            ...
            {
                if(XButton>=30 && XButton<=750 && YButton>=45 && YButton<=675)
                {
                    if(YButton<=165) choice=1;
                    if(YButton>=225 && YButton<=405 && XButton<=360) choice=2;
                    if(YButton>=225 && YButton<=405 && XButton>=420) choice=3;
                    if(YButton>=480 && XButton<=360) choice=4;
                    if(YButton>=480 && XButton>=420) choice=0;
                }
                else if(XButton>=750 && XButton<=780 && YButton>=45 && YButton<=165) choice=1;
            }
        }
        ...
        if(choice==1)
        {
            ...
            boxdisplay("Which Pokémon do you want to know about?\n");
...
void initPokemon(void)
{
    ...
    addPokemon(76, "Golem", 80, 120, 130, 55, 65, 45, 8, 7, 0, "Rock Slide", "Earthquake", "Fire Blast", "Explosion");
    ...
    addPokemon(666, "Vivillon", 80, 52, 50, 90, 50, 89, 12, 13, 0, "Bug Bite", "Signal Beam", "Aerial Ace", "Energy Ball");
...
void initsound(void)
{
    ...
    music[1].setVolume(sound_volume/2);
    ...
    music[0].setVolume(sound_volume/2);
...
void changetrack(int id)
{
    ...
    {
        if(music[0].getPlayingOffset().asMilliseconds()<105892) music[0].setPlayingOffset(sf::milliseconds(105892));
        else if(music[0].getPlayingOffset().asMilliseconds()<194835) music[0].setPlayingOffset(sf::milliseconds(194835));
        else if(music[0].getPlayingOffset().asMilliseconds()<292012) music[0].setPlayingOffset(sf::milliseconds(292012));
        else if(music[0].getPlayingOffset().asMilliseconds()<384271) music[0].setPlayingOffset(sf::milliseconds(384271));
        else if(music[0].getPlayingOffset().asMilliseconds()<443009) music[0].setPlayingOffset(sf::milliseconds(443009));
        else if(music[0].getPlayingOffset().asMilliseconds()<550459) music[0].setPlayingOffset(sf::milliseconds(550459));
        else if(music[0].getPlayingOffset().asMilliseconds()<672794) music[0].setPlayingOffset(sf::milliseconds(672794));
        else if(music[0].getPlayingOffset().asMilliseconds()<768328) music[0].setPlayingOffset(sf::milliseconds(768328));
        else if(music[0].getPlayingOffset().asMilliseconds()<861749) music[0].setPlayingOffset(sf::milliseconds(861749));
        else if(music[0].getPlayingOffset().asMilliseconds()<976276) music[0].setPlayingOffset(sf::milliseconds(976276));
        else if(music[0].getPlayingOffset().asMilliseconds()<1106722) music[0].setPlayingOffset(sf::milliseconds(1106722));
        else if(music[0].getPlayingOffset().asMilliseconds()<1256865) music[0].setPlayingOffset(sf::milliseconds(1256865));
        else if(music[0].getPlayingOffset().asMilliseconds()<1356853) music[0].setPlayingOffset(sf::milliseconds(1356853));
        else if(music[0].getPlayingOffset().asMilliseconds()<1473174) music[0].setPlayingOffset(sf::milliseconds(1473174));
        else if(music[0].getPlayingOffset().asMilliseconds()<1578979) music[0].setPlayingOffset(sf::milliseconds(1578979));
        else if(music[0].getPlayingOffset().asMilliseconds()<1711860) music[0].setPlayingOffset(sf::milliseconds(1711860));
...
void pokeinfo(pokeprofile *p)
{
    ...
    sprite.scale(2, 2);
    sprite.setPosition(330, 300);

    int flag=1;
    clk.restart();
    while(clk.getElapsedTime().asSeconds()<30 && flag && window.isOpen())
    {
        window.clear();
        window.draw(bgs);
        window.draw(sprite);
        puttext(std::to_string(p->dexid)+". "+name, 180, 20, 48);
        ...
        while(window.pollEvent(event))
        {
            ...
            if(event.type==sf::Event::MouseButtonPressed && event.mouseButton.button==sf::Mouse::Left)
            {
                if(XButton>=150 && XButton<=510 && YButton>=120 && YButton<=480)
                {
                    cry(p);
                    clk.restart();
                }
...
void moveinfo(pokemove *p)
{
    ...
    while(clk.getElapsedTime().asSeconds()<30 && flag && window.isOpen())
    {
        window.clear();
        window.draw(sprite);
        puttext("Move:", 90, 20, 45);
        puttext(name, 300, 20, 45);
        puttext("Type:", 90, 200, 40);
        puttext(type, 300, 200, 40);
        puttext("Category:", 90, 310, 40);
        puttext(cat, 300, 310, 40);
        puttext("Power:", 780, 200, 36);
        puttext(std::to_string(p->power), 990, 200, 36);
        puttext("Accuracy:", 780, 275, 36);
        puttext(std::to_string(p->accuracy>100? 100: p->accuracy), 990, 275, 36);
        puttext("PP:", 780, 350, 36);
        puttext(std::to_string(p->pp), 990, 350, 36);
...
void pokelist(void)
{
    ...
    sf::Text text;
    text.setString("Click on any Pokémon to see info.\nUse the Arrow keys or Mouse to scroll. Press any other key to exit.");
    text.setFont(arial);
    ...
    sf::RectangleShape shape(sf::Vector2f(170, 140));
    shape.setOrigin(90, 80);
    ...
    for(i=0; i<21; i++)
    {
        t[i].loadFromFile("img/sprites/"+std::to_string(gotonum(p+i)->dexid)+".png");
        s[i].setTexture(t[i]);

        std::string str(gotonum(p+i)->name);
        txt[i].setString(std::to_string(gotonum(p+i)->dexid)+". "+str);

        s[i].setOrigin(120, 120);
        s[i].setPosition(100+180*(i%7), 240+180*(i/7));

        txt[i].setFont(arial);
        txt[i].setCharacterSize(18);
        txt[i].setFillColor(sf::Color::White);
        txt[i].setOutlineThickness(1);
        txt[i].setOutlineColor(sf::Color::Black);
        txt[i].setStyle(sf::Text::Bold);
        txt[i].setPosition(30+180*(i%7), 310+180*(i/7));
    }
    ...
    while(window.isOpen() && flag)
    {
        ...
        window.draw(text);
        window.draw(shape);
        for(i=0; i<21; i++) window.draw(s[i]);
        for(i=0; i<21; i++) window.draw(txt[i]);
        window.display();
        ...
        while(window.pollEvent(event))
        {
            ...
            if(event.type==sf::Event::KeyPressed)
            {
                ...
                else if(event.key.code==sf::Keyboard::Down && p<343) p+=7;
                else if(event.key.code==sf::Keyboard::Down && p==343) p++;
                else if(event.key.code==sf::Keyboard::Up && p==344) p--;
                else if(event.key.code==sf::Keyboard::Up && p>0) p-=7;

                else if(event.key.code==sf::Keyboard::Right && p<329) p+=21;
                else if(event.key.code==sf::Keyboard::Right) p=344;
                else if(event.key.code==sf::Keyboard::Left && p==344) p--;
                else if(event.key.code==sf::Keyboard::Left && p>14) p-=21;
                else if(event.key.code==sf::Keyboard::Left) p=0;
            ...
            {
                if(event.mouseWheelScroll.delta<0 && p<343) p+=7;
                else if(event.mouseWheelScroll.delta<0 && p==343) p++;
                else if(event.mouseWheelScroll.delta>0 && p==344) p--;
                else if(event.mouseWheelScroll.delta>0 && p>0) p-=7;
            ...
            {
                if(event.mouseButton.button==sf::Mouse::Left && YButton<150) flag=0;
                else if(XButton>=10 && XButton<=1270 && YButton>=150) pokeinfo(gotonum(p+7*((YButton-150)/180)+((XButton-10)/180)));
            }

            if(event.type==sf::Event::MouseMoved)
            {
                if(XCursor>=10 && XCursor<=1270 && YCursor>=150)
                {
                    shape.setOutlineThickness(2);
                    shape.setPosition(100+180*((XCursor-10)/180), 240+180*((YCursor-150)/180));
                }

                else shape.setOutlineThickness(0);
            }
        }
    }
}
...
int typefx(pokeprofile *def, pokemove *mv)
{
    ...
    else if(a==7) //Ground Type Move
    {
        ...
        if(d1==2 || d2==2) m/=2; //Not very effective against Grass
        if(d1==12 || d2==12) m/=2; //Not very effective against Bug
        if(d1==13 || d2==13 || def->levitate) return 0; //Ineffective against Flying and Pokémon with Levitate ability
    }
...
void teambuilder(bool pos)
{
    ...
    while(!choice && window.isOpen())
    {
        ...
        while(window.pollEvent(event))
        {
            ...
            if(event.type==sf::Event::MouseButtonPressed && event.mouseButton.button==sf::Mouse::Left && XButton>=310 && XButton<=470)
            {
                if(YButton>=590 && YButton<=620) choice=1;
                if(YButton>=630 && YButton<=660) choice=2;
            }
        }
    }
    msg="";
    ...
    else
    {
        ...
        else
        {
            ...
            else if(!strncmp(trainer[pos].name, "Leaf", 4)) trainer[pos].photo=2*((rand()&1)+1);
    ...
    boxdisplay("Remember, ...");
    boxdisplay("You have to press the Space key to see the list of available Pokémon species.");

    int i, j, id=0, diff;
    for(i=0; i<6; ++i, id=0)
    {
        boxdisplay(std::to_string(i+1)+". Choose A Pokémon\nTrainer "+name+"-\n");
...
void choosePokemon(bool pos)
{
    ...
    while(1)
    {
        ...
        textdisplay("Choose Your Active Pokémon, "+str+"-");
    ...
    while(!temp)
    {
        while(window.pollEvent(event))
        {
            ...
            if(event.type==sf::Event::MouseMoved)
            {
                if(YCursor>=600 && XCursor>=40 && XCursor<1240)
                {
                    high=(XCursor-40)/200;
            ...
            if(event.type==sf::Event::MouseButtonPressed && event.mouseButton.button==sf::Mouse::Left && YButton>=600)
            {
                if(XButton>=40 && XButton<1240) id=(XButton-40)/200;
                temp=trainer[pos].team[id].hp;
            }
        }
    }
    ...
    if(trainer[pos].team[id].dex==gotoPokemon("Greninja") && music[1].getStatus()!=sf::Music::Playing && trainer[pos].team[id].stat[1]>250 && rand()&1)
    {
        music[0].pause();
        music[0].setPlayingOffset(sf::milliseconds(1356853));
    }
}
...
void attack(bool pos)
{
    ...
    int i=4, damage=0, a=1, d=3, high=0;
    ...
    while(i>3 && window.isOpen())
    {
        while(window.pollEvent(event))
        {
            ...
            if(event.type==sf::Event::MouseButtonPressed && event.mouseButton.button==sf::Mouse::Left && YButton>=600 && YButton<=700)
            {
                if(XButton>=120 && XButton<=600) i=0;
                else if(XButton>=760 && XButton<=1240) i=1;
                i+=2*(YButton>=650);
            }
        }
    }
...
bool lose(player *trainer)
{
    int trh=0; //TotalRemainingHP of all Pokémon
...
void puttext(std::string txt, int x, int y, int size)
{
    ...
    for(int i=0; i<str.length(); i++)
    {
        ...
        if(text.findCharacterPos(i).y+size>625)
        {
            puttext(txt, x, y, size-1);
            return;
        }
